...
import { LocationEditForm } from './components/modals/LocationEditForm'
import { QuestEditForm } from './components/modals/QuestEditForm'
import { NPCEditForm } from './components/modals/NPCEditForm'
import { ItemEditForm } from './components/modals/ItemEditForm'
import { PortalEditForm } from './components/modals/PortalEditForm'
import { EntityTooltip, TooltipEntity } from './components/ui/EntityTooltip'
import { DialogueEditForm } from './components/modals/DialogueEditForm'
...
export const EditModal: React.FC<EditModalProps> = ({
  ...
  useEffect(() => {
    if (formData && (entityType === 'npc' || entityType === 'item' || entityType === 'portal')) {
  ...
  const handleUpdateQuestStep = (questId: string, stepId: string, updates: Partial<QuestStep>) => {
    if (!onUpdateQuest) return

    const quest = availableQuests.find((q) => q.id === questId)
    if (!quest) return

    const updatedSteps = quest.steps.map((step) => (step.id === stepId ? { ...step, ...updates } : step))
  ...
  const openItemSelector = () => {
    ...
    const filteredItems = availableItems.filter((item) => !existingItemIds.includes(item.id))
  ...
  const openPortalSelector = () => {
    ...
    const filteredPortals = availablePortals.filter((portal) => !existingPortalIds.includes(portal.id))
  ...
  const selectPortal = (portal: Portal) => {
    ...
    const portalExists = location.portals.some((existingPortal) => existingPortal.id === portal.id)
  ...
  const openNpcSelector = () => {
    ...
    const filteredNpcs = availableNpcs.filter((npc) => !existingNpcIds.includes(npc.id))
  ...
  const selectNpc = (npc: NPC) => {
    ...
    const npcExists = location.npcs.some((existingNpc) => existingNpc.id === npc.id)
  ...
  const selectItem = (item: Item) => {
    ...
    const itemExists = location.items.some((existingItem) => existingItem.id === item.id)
  ...
  const addMultipleItemsToLocation = (items: Item[]) => {
    ...
    items.forEach((item) => {
      // Check if item is already in the location
      const itemExists = newItems.some((existingItem) => existingItem.id === item.id)
  ...
  const addMultiplePortalsToLocation = (portals: Portal[]) => {
    ...
    portals.forEach((portal) => {
      // Check if portal is already in the location
      const portalExists = newPortals.some((existingPortal) => existingPortal.id === portal.id)
  ...
  const openLocationSelector = () => {
    ...
    setLocationSelector({
      ...
      hoveredLocation: null,
      mousePosition: { x: 0, y: 0 },
      selectedLocation: availableLocations.find((loc) => loc.id === portal.destinationLocationId) || null
    })
  }
    ...
    return (
      <div className="edit-form">
        <div className="form-group">
          <label>ID:</label>
          <input type="text" value={location.id} readOnly style={{ backgroundColor: '#333', cursor: 'not-allowed' }} />
        </div>
        <div className="form-group">
          <label>Name:</label>
          <input type="text" value={location.name} onChange={(e) => updateFormData({ name: e.target.value })} />
        </div>
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) => updateFormData({ backgroundMusic: e.target.value })}
        ...
        <div className="form-group">
          ...
          <div className="array-list">
            ...
            <button type="button" className="add-array-button" onClick={openItemSelector}>
              Add Item
            </button>
          </div>
        </div>
        <div className="form-group">
          ...
          <div className="array-list">
              ...
              <div key={index} className="array-item location-npc">
                <div className="npc-display">
                  <div className="npc-icon">
                    {npc.image ? (
                      <img src={npc.image} alt={npc.name} className="npc-thumbnail" />
                    ) : (
                      <div className="no-thumbnail">?</div>
                    )}
                  </div>
            ...
            <button type="button" className="add-array-button" onClick={openNpcSelector}>
              Add NPC
            </button>
          </div>
        </div>
        <div className="form-group">
          ...
          <div className="array-list">
              ...
              <div key={index} className="array-item location-portal">
                <div className="portal-display">
                  <div className="portal-icon">
                    {portal.image ? (
                      <img src={portal.image} alt={portal.name} className="portal-thumbnail" />
                    ) : (
                      <div className="no-thumbnail">?</div>
                    )}
                  </div>
            ...
            <button type="button" className="add-array-button" onClick={openPortalSelector}>
              Add Portal
            </button>
          </div>
        </div>
  ...
  const renderNPCForm = () => {
    ...
    return (
      <div className="edit-form">
        <div className="form-group">
          <label>ID:</label>
          <input type="text" value={npc.id} readOnly style={{ backgroundColor: '#333', cursor: 'not-allowed' }} />
        </div>
        <div className="form-group">
          <label>Name:</label>
          <input type="text" value={npc.name} onChange={(e) => updateFormData({ name: e.target.value })} />
        </div>
        <div className="form-group">
          <label>Image:</label>
          <input
  ...
  const renderItemForm = () => {
    ...
    return (
      <div className="edit-form">
        <div className="form-group">
          <label>ID:</label>
          <input type="text" value={item.id} readOnly style={{ backgroundColor: '#333', cursor: 'not-allowed' }} />
        </div>
        <div className="form-group">
          <label>Name:</label>
          <input type="text" value={item.name} onChange={(e) => updateFormData({ name: e.target.value })} />
        </div>
        <div className="form-group">
          <label>Image:</label>
          <input
        ...
        <div className="form-group">
          ...
          <select
            ...
            onChange={(e) => updateFormData({ interactive: e.target.value as InteractiveMode })}
          >
  ...
  const renderPortalForm = () => {
    ...
    const selectedLocation = availableLocations.find((loc) => loc.id === portal.destinationLocationId)
      ...
      <div className="edit-form">
        ...
        <div className="form-group">
          ...
          <div className="location-selector-container">
            <div className="item-selector-display" onClick={openLocationSelector}>
              {selectedLocation ? (
                <div className="selected-location">
                  <div className="item-icon">
                    {selectedLocation.image ? (
                      <img src={selectedLocation.image} alt={selectedLocation.name} className="item-thumbnail" />
                    ) : (
                      <div className="no-thumbnail">?</div>
                    )}
                  </div>
                ...
                <div className="no-location-selected">Click to select destination location</div>
              )}
  ...
  const renderItemSelector = () => {
    ...
    return (
      <div className="modal-overlay" onClick={closeItemSelector}>
        <div className="modal-content item-selector" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="selector-help">
              ...
              {itemSelector.selectedItems.length > 0 && (
                <p className="selection-count">
                  {itemSelector.selectedItems.length} item{itemSelector.selectedItems.length !== 1 ? 's' : ''} selected
                </p>
              )}
            </div>
            {itemSelector.availableItems.length === 0 ? (
              <div className="no-items">
                <p>No items available or all items are already added to this location.</p>
              </div>
            ) : (
              <div className="item-grid">
                {itemSelector.availableItems.map((item) => (
                  <div
                    key={item.id}
                    className={`item-slot ${itemSelector.selectedItems.some((selected) => selected.id === item.id) ? 'selected' : ''}`}
                    onClick={(e) => {
        ...
        {unifiedTooltip.entity && unifiedTooltip.position && (
          <EntityTooltip entity={unifiedTooltip.entity} position={unifiedTooltip.position} visible={true}>
            <div></div>
          </EntityTooltip>
        )}
      </div>
                ...
                {portalSelector.availablePortals.map((portal) => (
                  <div
                    ...
                    onClick={(e) => {
                      if (portalSelector.isShiftPressed) {
                        // Multi-selection mode
                        setPortalSelector((prev) => {
                          const isSelected = prev.selectedPortals.some((selected) => selected.id === portal.id)
                          if (isSelected) {
                            // Remove from selection
                            return {
                              ...prev,
                              selectedPortals: prev.selectedPortals.filter((selected) => selected.id !== portal.id)
                            }
                          } else {
                            // Add to selection
                            return {
                              ...prev,
                              selectedPortals: [...prev.selectedPortals, portal]
                            }
                          }
                        })
                      } else {
                  ...
                  >
                    <div className="portal-image-container">
                      {portal.image ? (
                        <img src={portal.image} alt={portal.name} className="portal-image" />
                      ) : (
                        <div className="no-image">No Image</div>
                      )}
                    </div>
  ...
  const renderNpcSelector = () => {
    ...
    return (
      <div className="modal-overlay" onClick={closeNpcSelector}>
        <div className="modal-content npc-selector" onClick={(e) => e.stopPropagation()}>
          <div className="modal-header">
          ...
          <div className="modal-body">
            <div className="selector-help">
              <p>
                💡 Hold <kbd>Shift</kbd> to select multiple NPCs. Release to add all selected NPCs.
              </p>
              {npcSelector.selectedNpcs.length > 0 && (
                <p className="selection-count">
                  {npcSelector.selectedNpcs.length} NPC{npcSelector.selectedNpcs.length !== 1 ? 's' : ''} selected
                </p>
              )}
            </div>
            {npcSelector.availableNpcs.length === 0 ? (
              <div className="no-npcs">
                <p>No NPCs available or all NPCs are already added to this location.</p>
              </div>
            ) : (
              <div className="npc-grid">
                {npcSelector.availableNpcs.map((npc) => (
                  <div
                    key={npc.id}
                    className={`npc-slot ${npcSelector.selectedNpcs.some((selected) => selected.id === npc.id) ? 'selected' : ''}`}
                    onClick={(e) => {
                      if (npcSelector.isShiftPressed) {
                        // Multi-selection mode
                        setNpcSelector((prev) => {
                          const isSelected = prev.selectedNpcs.some((selected) => selected.id === npc.id)
                          if (isSelected) {
                            // Remove from selection
                            return {
                              ...prev,
                              selectedNpcs: prev.selectedNpcs.filter((selected) => selected.id !== npc.id)
                            }
                          } else {
                            // Add to selection
                            return {
  ...
  const renderLocationSelector = () => {
    ...
    return (
      <div className="modal-overlay" onClick={closeLocationSelector}>
        <div className="modal-content location-selector" onClick={(e) => e.stopPropagation()}>
          <div className="modal-header">
          ...
          <div className="modal-body">
            <div className="selector-help">
              ...
              {locationSelector.selectedLocation && (
                <p className="selection-count">
                  Selected: <strong>{locationSelector.selectedLocation.name}</strong>
                </p>
              )}
            </div>
            {locationSelector.availableLocations.length === 0 ? (
              ...
              <div className="item-grid">
                {locationSelector.availableLocations.map((location) => (
                  <div
                    key={location.id}
                    className={`item-slot ${locationSelector.selectedLocation?.id === location.id ? 'selected' : ''}`}
                  ...
                  >
                    <div className="item-image-container">
                      {location.image ? (
                        <img src={location.image} alt={location.name} className="item-image" />
                      ) : (
                        <div className="no-image">No Image</div>
                      )}
                    </div>
  ...
  const renderDialogueEditor = () => {
    ...
    const updateButton = (dialogIndex: number, buttonIndex: number, updates: Partial<Button>) => {
      const dialog = sequence.dialogs[dialogIndex]
    ...
    const removeButton = (dialogIndex: number, buttonIndex: number) => {
      const dialog = sequence.dialogs[dialogIndex]
      const updatedButtons = (dialog.buttons || []).filter((_, i) => i !== buttonIndex)
      updateDialog(dialogIndex, { buttons: updatedButtons })
    }
        ...
        <div className="modal-content dialogue-editor modal-content-dialogue" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="modal-body">
            <div className="form-group">
              ...
              <input type="text" value={sequence.name} onChange={(e) => updateSequence({ name: e.target.value })} />
            </div>
            <div className="form-group">
              ...
              <div className="dialogs-list">
                {sequence.dialogs.map((dialog, dialogIndex) => (
                  <div key={dialog.id} className="dialog-item">
                    <div className="dialog-header">
                      ...
                      <button type="button" className="remove-button small" onClick={() => removeDialog(dialogIndex)}>
                        Remove
                      </button>
                    </div>
                    <div className="dialog-fields">
                      <div className="field-group">
                        ...
                        <textarea
                          value={dialog.text}
                          onChange={(e) => updateDialog(dialogIndex, { text: e.target.value })}
                          rows={3}
                        />
                      </div>
                      <div className="field-row">
                        <div className="dialog-type-buttons">
                          <button
                            ...
                            onClick={() => updateDialog(dialogIndex, { isQuestion: true, isEndOfDialog: false })}
                          >
                            Is Question
                          </button>
                          <button
                            ...
                            onClick={() => updateDialog(dialogIndex, { isQuestion: false, isEndOfDialog: true })}
                          >
                            End of Dialog
                          </button>
                          <button
                            ...
                            onClick={() => updateDialog(dialogIndex, { isQuestion: false, isEndOfDialog: false })}
                          >
                            Continue
                          </button>
                        </div>
                            ...
                            {(dialog.buttons || []).map((button, buttonIndex) => (
                              <div key={buttonIndex} className="button-item">
                                <input
                                  ...
                                  value={button.label}
                                  onChange={(e) => updateButton(dialogIndex, buttonIndex, { label: e.target.value })}
                                />
                                <select
                                  value={button.goToDialog}
                                  onChange={(e) =>
                                    updateButton(dialogIndex, buttonIndex, { goToDialog: parseInt(e.target.value) })
                                  }
                                >
                                ...
                                <input
                                  ...
                                  value={button.size}
                                  onChange={(e) =>
                                    updateButton(dialogIndex, buttonIndex, { size: parseInt(e.target.value) || 300 })
                                  }
                                  min="100"
                                  max="1000"
                                  style={{ width: '80px' }}
                                ...
                                <button
                                  ...
                                  onClick={() => removeButton(dialogIndex, buttonIndex)}
                                >
                                  Remove
                                </button>
                              </div>
                            ))}
                            <button type="button" className="add-button small" onClick={() => addButton(dialogIndex)}>
                              Add Button
                            </button>
                          </div>
                        </div>
                ...
                <button type="button" className="add-array-button" onClick={addDialog}>
                  Add Dialog
                </button>
              </div>
            </div>
          ...
          <div className="modal-footer">
            <button className="modal-button cancel" onClick={closeDialogueEditor}>
              Cancel
            </button>
            <button className="modal-button save" onClick={() => saveDialogueSequence(sequence)}>
              Save Sequence
            </button>
          </div>
        </div>
  ...
  const renderForm = () => {
    switch (entityType) {
      ...
      case 'item':
        return <ItemEditForm item={formData as Item} onUpdate={updateFormData} />
      case 'portal':
        return (
          <PortalEditForm
      ...
      case 'dialogue':
        ...
        return (
          <DialogueEditForm
            ...
            onUpdate={updateFormData}
            onOpenDialogueEditor={() => openDialogueEditor(formData as DialogueSequence)}
  ...
  return (
    <>
      <div className="modal-overlay" onClick={handleClose}>
        <div
          className={`modal-content ${entityType === 'dialogue' ? 'modal-content-dialogue' : ''}`}
          onClick={(e) => e.stopPropagation()}
        >
          <div className="modal-header">
            <h2>
              Edit {entityType.charAt(0).toUpperCase() + entityType.slice(1)}: {formData?.id}
            </h2>
