  ...
  return (
    <EntityPanel
      ...
      renderEntity={(dialogue: DialogueSequence) => {
        const npc = dialogue.npcId ? npcs.find((n) => n.id === dialogue.npcId) : null

        // Get quest step assigned to this dialogue
        const allQuestSteps = quests.flatMap((quest) =>
          quest.steps.map((step) => ({ ...step, questId: quest.id, questTitle: quest.title }))
        )
        ...
        return (
          <div className="entity-card dialogue-card">
            <div className="npc-portrait">
              {npc?.image ? (
                <img src={npc.image} alt={`${npc.name} image`} className="portrait-image" />
              ) : (
            ...
            <div className="dialogue-details">
              ...
              <div className="assigned-quest-steps">
                ...
                {assignedQuestSteps.length > 2 && (
                  <p className="more-steps">... and {assignedQuestSteps.length - 2} more</p>
                )}
              </div>
            </div>
          </div>
