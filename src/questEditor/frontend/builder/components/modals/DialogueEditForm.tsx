...
interface DialogueEditFormProps {
  ...
  onUpdateQuestStep?: (questId: string, stepId: string, updates: Partial<QuestStep>) => void
  availableNpcs?: NPC[]
  ...
  return (
    <div className="dialogue-edit-form">
        ...
        <div className="form-group npc-display">
          ...
          <div className="npc-info">
            <div className="npc-portrait">
              {npc.image ? (
                <img src={npc.image} alt={`${npc.name} portrait`} className="portrait-image" />
              ) : (
      ...
      <div className="form-group">
            ...
            <div className="quest-step-item">
              <div className="quest-step-info">
                <span className="quest-title">
                  {availableQuests.find((q) => q.steps.some((s) => s.id === assignedQuestStep.id))?.title}:
                </span>
                <span className="step-name">{assignedQuestStep.name}</span>
                <span className="step-id">({assignedQuestStep.id})</span>
              </div>
              <button type="button" className="remove-step-button" onClick={handleDeselectQuestStep}>
                ×
              </button>
            </div>
          ) : (
        ...
        {!assignedQuestStep && (
          <button type="button" className="add-quest-step-button" onClick={() => setShowQuestStepSelector(true)}>
            + Assign Quest Step
          </button>
        )}
      </div>
      ...
      <div className="form-group">
              ...
              {dialogue.dialogs.slice(0, 3).map((dialog, index) => (
                <div key={dialog.id} className="dialogue-item">
                  <span className="dialogue-text">
                    {dialog.text.length > 50 ? `${dialog.text.substring(0, 50)}...` : dialog.text}
                  </span>
                  {dialog.isQuestion && <span className="dialogue-badge question">Question</span>}
                  {dialog.isEndOfDialog && <span className="dialogue-badge end">End</span>}
                </div>
              ))}
              {dialogue.dialogs.length > 3 && (
                <div className="dialogue-more">... and {dialogue.dialogs.length - 3} more dialogs</div>
              )}
            </div>
          ) : (
        ...
        <button type="button" className="edit-dialogue-button" onClick={onOpenDialogueEditor}>
