...
export const QuestStepEditModal: React.FC<QuestStepEditModalProps> = ({
  ...
  useEffect(() => {
    if (formData.name) {
      ...
      const timeoutId = setTimeout(async () => {
        try {
          const response = await generateIdFromApi(formData.name, 'quest-step', currentId)
  ...
  const updateAction = (target: 'onStart' | 'onComplete', index: number, action: Action) => {
    setFormData((prev) => ({
      ...prev,
          ...
          <div className="actions-list">
            {formData.onStart.map((action, index) => (
              <ActionEditor
                key={index}
                action={action}
                onChange={(newAction) => updateAction('onStart', index, newAction)}
            ...
            <button type="button" className="add-action-button" onClick={() => addAction('onStart')}>
              Add On Start Action
            </button>
          </div>
        </div>
            ...
            <div className="entity-selector">
              {[...items, ...npcs].map((entity) => (
                <label key={entity.id} className="entity-checkbox">
                  <input
                    type="checkbox"
                    checked={(formData.objectiveParams.entityIds || []).includes(entity.id)}
                    onChange={(e) => {
                      const currentIds = formData.objectiveParams.entityIds || []
                      if (e.target.checked) {
                        updateObjectiveParams('entityIds', [...currentIds, entity.id])
                      } else {
          ...
          <>
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) => updateObjectiveParams('itemName', e.target.value)}
              >
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) => updateObjectiveParams('count', parseInt(e.target.value) || 1)}
              />
            </div>
          </>
        )}
        ...
        {formData.objectiveType === 'goToLocation' && (
          <div className="form-group">
            ...
            <select
              ...
              onChange={(e) => updateObjectiveParams('locationId', e.target.value)}
            >
          ...
          <div className="actions-list">
            {formData.onComplete.map((action, index) => (
              <ActionEditor
                key={index}
                action={action}
                onChange={(newAction) => updateAction('onComplete', index, newAction)}
            ...
            <button type="button" className="add-action-button" onClick={() => addAction('onComplete')}>
...
const ActionEditor: React.FC<ActionEditorProps> = ({
  ...
  return (
    <div className="action-editor">
      <select value={action.type} onChange={(e) => updateType(e.target.value as ActionType)}>
        <option value={ActionType.PlaySound}>Play Sound</option>
        <option value={ActionType.AddToInventory}>Add to Inventory (World → Inventory)</option>
        <option value={ActionType.GrantToInventory}>Grant to Inventory (Void → Inventory)</option>
        <option value={ActionType.RemoveFromInventory}>Remove from Inventory (by ID)</option>
        <option value={ActionType.RemoveFromInventoryByName}>Remove from Inventory (by Name)</option>
        <option value={ActionType.SpawnEntity}>Spawn Entity (Void → World)</option>
        <option value={ActionType.ClearEntity}>Clear Entity (World → Void)</option>
        <option value={ActionType.SetInteractive}>Set Interactive Mode (by ID)</option>
        <option value={ActionType.SetInteractiveByName}>Set Interactive Mode (by Name)</option>
      ...
      {action.type === ActionType.AddToInventory && (
        <select value={action.params.entityId || ''} onChange={(e) => updateParam('entityId', e.target.value)}>
      ...
      {action.type === ActionType.SetInteractive && (
        <>
          ...
          <select
            ...
            onChange={(e) => updateParam('interactiveMode', e.target.value as InteractiveMode)}
          >
      ...
      {action.type === ActionType.SetInteractiveByName && (
        <>
          <select value={action.params.itemName || ''} onChange={(e) => updateParam('itemName', e.target.value)}>
      ...
      {action.type === ActionType.ActivateQuest && (
        <select value={action.params.questId || ''} onChange={(e) => updateParam('questId', e.target.value)}>
      ...
      {action.type === ActionType.ChangeLocation && (
        <select value={action.params.locationId || ''} onChange={(e) => updateParam('locationId', e.target.value)}>
