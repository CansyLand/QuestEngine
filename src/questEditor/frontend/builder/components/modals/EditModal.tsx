...
import { EntityType, ItemSelectorState, DialogueEditorState } from '../../types/index.js'
import { LocationEditForm } from './LocationEditForm.js'
import { QuestEditForm } from './QuestEditForm.js'
import { NPCEditForm } from './NPCEditForm.js'
import { ItemEditForm } from './ItemEditForm.js'
import { PortalEditForm } from './PortalEditForm.js'
import { ItemSelector } from '../dialogs/ItemSelector.js'
import { DialogueEditor } from '../dialogs/DialogueEditor.js'
import { UniversalSelector } from '../ui/UniversalSelector.js'
...
export const EditModal: React.FC<EditModalProps> = ({
  ...
  const [formData, setFormData] = useState<Location | Quest | NPC | Item | Portal | null>(null)
  ...
  const updateFormData = (updates: Partial<Location | Quest | NPC | Item | Portal>) => {
  ...
  const removeDialog = (index: number) => {
    ...
    const updatedDialogs = dialogueEditor.sequence.dialogs.filter((_: Dialog, i: number) => i !== index)
    updateDialogueSequence({ dialogs: updatedDialogs })
  }
  ...
  const removeButton = (dialogIndex: number, buttonIndex: number) => {
    ...
    const dialog = dialogueEditor.sequence.dialogs[dialogIndex]
    const updatedButtons = (dialog.buttons || []).filter((_: Button, i: number) => i !== buttonIndex)
    updateDialog(dialogIndex, { buttons: updatedButtons })
  }
      ...
      <div className="modal-overlay" onClick={onClose}>
        <div className="modal-content edit-modal" onClick={(e) => e.stopPropagation()}>
          <div className="modal-header">
            <h2>Edit {entityType.charAt(0).toUpperCase() + entityType.slice(1)}</h2>
          ...
          <div className="modal-body">
            ...
            {entityType === 'npc' && <NPCEditForm npc={formData as NPC} onUpdate={updateFormData} />}
            {entityType === 'item' && <ItemEditForm item={formData as Item} onUpdate={updateFormData} />}
            {entityType === 'portal' && (
              <PortalEditForm
