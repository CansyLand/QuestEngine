...
const EntityDisplay = <T extends BaseEntity>({
  ...
  const containerClasses = `entity-display entity-${variant} ${className}`.trim()
  ...
  return (
    <>
      ...
      >
        {showImage && (
          <div className={`entity-image entity-${typeLabel.toLowerCase()}-image`}>
            {imageUrl ? (
              <img src={imageUrl} alt={`${entity.name} image`} className="entity-thumbnail" />
            ) : (
...
export const ItemDisplay: React.FC<Omit<EntityDisplayProps<Item>, 'entity'> & { entity: Item }> = (props) => (
  <EntityDisplay {...props} />
)

export const NPCDisplay: React.FC<Omit<EntityDisplayProps<NPC>, 'entity'> & { entity: NPC }> = (props) => (
  <EntityDisplay {...props} />
)

export const PortalDisplay: React.FC<Omit<EntityDisplayProps<Portal>, 'entity'> & { entity: Portal }> = (props) => (
  <EntityDisplay {...props} />
)

export const LocationDisplay: React.FC<Omit<EntityDisplayProps<Location>, 'entity'> & { entity: Location }> = (
  props
) => <EntityDisplay {...props} />
