...
class QuestEditorIntegration {
        ...
        const monitorChanges = async () => {
            try {
                ...
                if (currentHash !== lastHash) {
                    ...
                    await this.updateLinks(newLinks, linksPath);
                }
            }
            catch (error) {
    ...
    extractEntityData(compositeData) {
        const links = {};
        const transformData = compositeData.components?.find((c) => c.name === 'core::Transform')?.data || {};
        const nameData = compositeData.components?.find((c) => c.name === 'core-schema::Name')?.data || {};
     ...
     */
    async updateLinks(newLinks, linksPath) {
        let existingLinks = {};
        try {
        ...
        for (const entityId in existingLinks) {
            ...
            if (newLinks[entityId]) {
                updatedLinks[entityId] = {
                    ...newLinks[entityId],
                    questEntityId: existingEntity.questEntityId
                };
            }
        }
        // Add new entities
        ...
        console.log('entityLinks.json updated for project');
    }
    /**
