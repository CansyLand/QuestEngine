...
function generateIdFromName(name, entityType, prefix) {
    if (!name || name.trim() === '') {
...
function generateUniqueId(name, entityType, existingIds, prefix) {
...
function createApiRouter(engine, persistence) {
    const router = express_1.default.Router();
    // Generate unique ID from name
    router.post('/generate-id', express_1.default.json(), (req, res) => {
        try {
            ...
            switch (entityType) {
                ...
                case 'quest-step':
                    ...
                    existingIds = gameData.quests.flatMap((quest) => quest.steps.map((step) => step.id));
                    break;
                default:
                    return res.status(400).json({
                        success: false,
    ...
    function updateEntityReferences(gameData, entityType, oldId, newId) {
        ...
        updatedData.quests.forEach((quest) => {
            quest.steps.forEach((step) => {
                if (step.onStart) {
                    step.onStart.forEach((action) => {
                        if (action.params) {
                            if (action.params.entityId === oldId)
                                action.params.entityId = newId;
                            if (action.params.itemName === oldId)
                                action.params.itemName = newId;
                            if (action.params.npcId === oldId)
                                action.params.npcId = newId;
                            if (action.params.portalId === oldId)
                                action.params.portalId = newId;
                            if (action.params.locationId === oldId)
                                action.params.locationId = newId;
                            if (action.params.questId === oldId)
                                action.params.questId = newId;
                        }
                    });
                }
                if (step.onComplete) {
                    step.onComplete.forEach((action) => {
                        if (action.params) {
                            if (action.params.entityId === oldId)
                                action.params.entityId = newId;
                            if (action.params.itemName === oldId)
        ...
        if (entityType === 'dialogue') {
            updatedData.npcs.forEach((npc) => {
                if (npc.dialogueSequences) {
                    npc.dialogueSequences = npc.dialogueSequences.map((dialogueId) => dialogueId === oldId ? newId : dialogueId);
                }
            });
        }
        ...
        if (entityType === 'quest-step') {
            ...
            updatedData.dialogues.forEach((dialogue) => {
    ...
    router.post('/save', express_1.default.json(), async (req, res) => {
        try {
            ...
            if (gameData.items) {
                const portalsInItems = gameData.items.filter((item) => item.destinationLocationId);
                if (portalsInItems.length > 0) {
                    console.warn(`API SAVE: Found ${portalsInItems.length} portals in items array before filtering:`, portalsInItems.map((p) => p.id));
                }
            }
            // Debug: Check for contamination in portals array
            if (gameData.portals) {
                const itemsInPortals = gameData.portals.filter((portal) => !portal.destinationLocationId);
                if (itemsInPortals.length > 0) {
                    console.warn(`API SAVE: Found ${itemsInPortals.length} items in portals array before filtering:`, itemsInPortals.map((p) => p.id));
                }
            }
            ...
            if (gameData.items) {
                ...
                gameData.items = gameData.items.filter((item) => !item.destinationLocationId);
                console.log(`API SAVE: Filtered items array, now contains ${gameData.items.length} items`);
            }
            if (gameData.portals) {
                // Filter out any items that might have been mixed into portals array
                gameData.portals = gameData.portals.filter((portal) => portal.destinationLocationId);
                console.log(`API SAVE: Filtered portals array, now contains ${gameData.portals.length} portals`);
            }
            // Load current state to detect ID changes
            const currentState = engine.getGameState();
            // Check for ID changes and update references
            const entityTypes = [
                { type: 'item', current: currentState.items, updated: gameData.items || [] },
                { type: 'npc', current: currentState.npcs, updated: gameData.npcs || [] },
                { type: 'location', current: currentState.locations, updated: gameData.locations || [] },
                { type: 'portal', current: currentState.portals, updated: gameData.portals || [] },
                { type: 'quest', current: currentState.quests, updated: gameData.quests || [] },
                { type: 'dialogue', current: currentState.dialogues, updated: gameData.dialogues || [] }
            ];
            let hasIdChanges = false;
            for (const { type, current, updated } of entityTypes) {
                for (const updatedEntity of updated) {
                    const currentEntity = current.find((e) => e.id === updatedEntity.id);
                    if (currentEntity) {
                        ...
                        let newName = '';
                        if (type === 'quest') {
                            const questCurrent = currentEntity;
                            const questUpdated = updatedEntity;
                            nameChanged = questCurrent.title !== questUpdated.title;
                            oldName = questCurrent.title;
                            newName = questUpdated.title;
                        }
                        else {
                            const nameCurrent = currentEntity;
                            const nameUpdated = updatedEntity;
                        ...
                        if (nameChanged && updatedEntity.id !== currentEntity.id) {
                            console.log(`ID change detected for ${type} ${currentEntity.id}: name changed from "${oldName}" to "${newName}", ID changed from "${currentEntity.id}" to "${updatedEntity.id}"`);
                            hasIdChanges = true;
                            // Update references throughout the data
                            gameData = updateEntityReferences(gameData, type, currentEntity.id, updatedEntity.id);
                        }
                    }
                }
            }
            ...
            for (const updatedQuest of gameData.quests || []) {
                const currentQuest = currentState.quests.find((q) => q.id === updatedQuest.id);
                if (currentQuest) {
                    for (const updatedStep of updatedQuest.steps) {
                        const currentStep = currentQuest.steps.find((s) => s.id === updatedStep.id);
                        if (currentStep && currentStep.name !== updatedStep.name && updatedStep.id !== currentStep.id) {
                            console.log(`ID change detected for quest-step ${currentStep.id}: name changed from "${currentStep.name}" to "${updatedStep.name}", ID changed from "${currentStep.id}" to "${updatedStep.id}"`);
                            hasIdChanges = true;
                            // Update references to the quest step ID
                            gameData = updateEntityReferences(gameData, 'quest-step', currentStep.id, updatedStep.id);
                        }
                    }
                }
            }
    ...
    router.post('/interact', express_1.default.json(), (req, res) => {
        try {
    ...
    router.post('/dialogue', express_1.default.json(), (req, res) => {
        try {
            const { dialogueSequenceId } = req.body;
            const gameState = engine.getGameState();
            const dialogue = gameState.dialogues.find((d) => d.id === dialogueSequenceId);
            if (!dialogue) {
    ...
    router.get('/entityLinks', (req, res) => {
        try {
            const linksPath = path_1.default.join(process.cwd(), 'questEditor/data/entityLinks.json');
            if (fs_1.default.existsSync(linksPath)) {
                const data = JSON.parse(fs_1.default.readFileSync(linksPath, 'utf8'));
                res.json(data);
            }
            else {
                res.json({});
    ...
    // Update entity link
    router.patch('/entityLinks/:entityId', express_1.default.json(), async (req, res) => {
        try {
            ...
            let data = {};
            if (fs_1.default.existsSync(linksPath)) {
                data = JSON.parse(fs_1.default.readFileSync(linksPath, 'utf8'));
            }
            if (data[entityId]) {
                console.log(`API: Updating entity ${entityId} questEntityId to ${questEntityId}`);
                data[entityId].questEntityId = questEntityId;
                console.log('API: Writing entityLinks.json file...');
                fs_1.default.writeFileSync(linksPath, JSON.stringify(data, null, 2));
                ...
                try {
                    ...
                    console.log('✅ Entity links updated and TypeScript compilation completed');
                }
                catch (compileError) {
                    console.error('❌ Failed to compile TypeScript after entity link update:', compileError);
