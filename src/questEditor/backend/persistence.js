	...
	compileDataToTypescript() {
		return new Promise((resolve, reject) => {
			try {
				...
				const questEngineDir = path.join(__dirname, '../../../') // src/questEngine
				const scriptPath = path.resolve(__dirname, './generate-data.js')
				// Store current working directory
				const originalCwd = process.cwd()
				try {
					// Change to questEngine directory (where generate-data.js expects to run)
					process.chdir(questEngineDir)
					console.log(`Changed working directory to: ${questEngineDir}`)
					// Execute the generate-data script directly in this process
					// Since it's already a Node.js environment, we can just require and run it
					const generateDataScript = require(scriptPath)
					// The script runs synchronously, so we can call it directly
					// Note: generate-data.js calls generateDataFile() at the end automatically
