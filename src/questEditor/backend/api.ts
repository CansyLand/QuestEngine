...
export function createApiRouter(engine: GameEngine, persistence: PersistenceManager) {
  ...
  function updateEntityReferences(gameData: Game, entityType: string, oldId: string, newId: string): Game {
      ...
      quest.steps.forEach((step: any) => {
        if (step.onStart) {
          step.onStart.forEach((action: any) => {
            if (action.params) {
              if (action.params.entityId === oldId) action.params.entityId = newId
              if (action.params.itemName === oldId) action.params.itemName = newId
              if (action.params.npcId === oldId) action.params.npcId = newId
              if (action.params.portalId === oldId) action.params.portalId = newId
              if (action.params.locationId === oldId) action.params.locationId = newId
        ...
        if (step.objectiveParams) {
          if (step.objectiveParams.npcId === oldId) step.objectiveParams.npcId = newId
          if (step.objectiveParams.itemName === oldId) step.objectiveParams.itemName = newId
          if (step.objectiveParams.portalId === oldId) step.objectiveParams.portalId = newId
        }
      })
    })
    ...
    if (entityType === 'dialogue') {
      updatedData.npcs.forEach((npc: any) => {
        if (npc.dialogueSequences) {
          npc.dialogueSequences = npc.dialogueSequences.map((dialogueId: string) =>
            dialogueId === oldId ? newId : dialogueId
          )
        }
      })
    }
  ...
  router.post('/save', express.json(), async (req, res) => {
    try {
      ...
      if (gameData.items) {
        const portalsInItems = gameData.items.filter((item: any) => item.destinationLocationId)
      ...
      if (gameData.portals) {
        const itemsInPortals = gameData.portals.filter((portal: any) => !portal.destinationLocationId)
      ...
      if (gameData.items) {
        ...
        gameData.items = gameData.items.filter((item: any) => !item.destinationLocationId)
      ...
      if (gameData.portals) {
        ...
        gameData.portals = gameData.portals.filter((portal: any) => portal.destinationLocationId)
      ...
      for (const { type, current, updated } of entityTypes) {
        for (const updatedEntity of updated) {
          const currentEntity = current.find((e: any) => e.id === updatedEntity.id)
          if (currentEntity) {
  ...
  router.get('/entityLinks', (req, res) => {
    try {
      const linksPath = path.join(process.cwd(), 'questEditor/data/entityLinks.json')
