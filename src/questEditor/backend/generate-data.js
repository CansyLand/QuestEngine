...
exports.compileDataToTypescriptShared = compileDataToTypescriptShared;
const fs = require('fs');
...
`;
    ...
    for (const [jsonFile, constantName] of Object.entries(fileMappings)) {
        ...
        try {
            ...
            console.error(`❌ Error processing ${jsonFile}:`, error instanceof Error ? error.message : String(error));
            process.exit(1);
        }
    }
...
async function compileDataToTypescriptShared(dataDir) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    console.warn('Could not restore original working directory:', cwdError);
                }
            }
        }
        catch (error) {
