...
class GameEngine {
    ...
    reset() {
        ...
        this.game.inventory.forEach((entityId) => {
            this.setEntityState(entityId, models_js_1.EntityState.Void);
        });
        this.game.inventory = [];
        // Reset active quests
    ...
    processInteraction(type, params) {
    ...
    handleItemInteraction(itemId) {
        ...
        if (!item || item.state !== models_js_1.EntityState.World)
            return commands;
        ...
        if (item.interactive === models_js_1.InteractiveMode.Grabbable) {
    ...
    handleNPCInteraction(npcId) {
        ...
        console.log(`Current location: ${currentLocation.id}, npcs:`, currentLocation.npcs.map((n) => n.id));
        const npc = currentLocation.npcs.find((n) => n.id === npcId);
        if (!npc) {
            console.log(`NPC ${npcId} not found in current location`);
            return commands;
        }
        if (npc.state !== models_js_1.EntityState.World) {
    ...
    handlePortalInteraction(portalId) {
        ...
        console.log(`Current location: ${currentLocation.id}, portals:`, currentLocation.portals.map((p) => p.id));
        const portal = currentLocation.portals.find((p) => p.id === portalId);
        if (!portal) {
            console.log(`Portal ${portalId} not found in current location`);
            return commands;
        }
        if (portal.state !== models_js_1.EntityState.World) {
    ...
    executeAction(action) {
        const commands = [];
        switch (action.type) {
            case models_js_1.ActionType.PlaySound:
                commands.push({
                    type: 'playSound',
                    params: { url: action.params.url }
                });
                break;
            case models_js_1.ActionType.AddToInventory:
                commands.push(...this.addToInventory(action.params.entityId));
                break;
            case models_js_1.ActionType.RemoveFromInventory:
                commands.push(...this.removeFromInventory(action.params.entityId));
                break;
            case models_js_1.ActionType.RemoveFromInventoryByName:
                commands.push(...this.removeFromInventoryByName(action.params.itemName, action.params.count));
                break;
            case models_js_1.ActionType.GrantToInventory:
                commands.push(...this.grantToInventory(action.params.entityId));
                break;
            case models_js_1.ActionType.SetInteractive:
                commands.push(...this.setInteractive(action.params.entityId, action.params.interactiveMode));
                break;
            case models_js_1.ActionType.SetInteractiveByName:
                commands.push(...this.setInteractiveByName(action.params.itemName, action.params.interactiveMode));
                break;
            case models_js_1.ActionType.SpawnEntity:
                commands.push({
                    type: 'spawnEntity',
                    params: { id: action.params.entityId }
                });
                this.setEntityState(action.params.entityId, models_js_1.EntityState.World);
                commands.push({
                    type: 'updateEntity',
                    params: { id: action.params.entityId, state: models_js_1.EntityState.World }
                });
                break;
            case models_js_1.ActionType.ClearEntity:
                commands.push({
                    type: 'clearEntity',
                    params: { id: action.params.entityId }
                });
                this.setEntityState(action.params.entityId, models_js_1.EntityState.Void);
                commands.push({
                    type: 'updateEntity',
                    params: { id: action.params.entityId, state: models_js_1.EntityState.Void }
                });
                break;
            case models_js_1.ActionType.ActivateQuest:
                commands.push(...this.activateQuest(action.params.questId));
                break;
            case models_js_1.ActionType.AdvanceStep:
                commands.push(...this.advanceStep());
                break;
            case models_js_1.ActionType.ChangeLocation:
                commands.push(...this.changeLocation(action.params.locationId));
                break;
            case models_js_1.ActionType.StartDialogue:
    ...
    handlePlaceSeed(vesselId) {
        ...
        if (seedInInventory) {
            ...
            commands.push(...this.setInteractive(vesselId, models_js_1.InteractiveMode.NotInteractive));
            // Log the action
            commands.push({
                type: 'log',
    ...
    addToInventory(entityId) {
        if (!this.game.inventory.includes(entityId)) {
            this.game.inventory.push(entityId);
            this.setEntityState(entityId, models_js_1.EntityState.Inventory);
        }
        return [
            {
            ...
            {
                type: 'updateEntity',
                params: { id: entityId, state: models_js_1.EntityState.Inventory }
            }
        ];
    }
    ...
    removeFromInventoryByName(itemName, count) {
    ...
    setInteractive(entityId, interactiveMode) {
    ...
    setInteractiveByName(itemName, interactiveMode) {
    ...
    startDialogue(dialogueSequenceId) {
        const commands = [];
    ...
    checkObjectives(interactedNpcId) {
        const commands = [];
        for (const questId of this.game.activeQuests) {
            const quest = this.game.quests.find((q) => q.id === questId);
            if (!quest || quest.completed)
            ...
            switch (currentStep.objectiveType) {
                case 'collectEntities':
                    ...
                    if (entityIds && entityIds.every((id) => this.game.inventory.includes(id))) {
    ...
    findDialogueForNPC(npcId) {
        // Find active quest steps that involve this NPC
        const relevantQuestSteps = [];
        for (const questId of this.game.activeQuests) {
            const quest = this.game.quests.find((q) => q.id === questId);
            if (!quest || quest.completed)
                continue;
