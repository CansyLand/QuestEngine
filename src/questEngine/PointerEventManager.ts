...
import { InteractiveMode, EntityState } from './models'
...
export class PointerEventManager {
	...
	updateEntityPointerEvents(entityId: string): void {
		...
		// Items and portals with interactive or grabbable modes
			...
			(questEntity.interactive === InteractiveMode.Interactive ||
				questEntity.interactive === InteractiveMode.Grabbable)
		) {
			console.log(
		...
		// Non-interactive entities
			...
			'interactive' in questEntity &&
			questEntity.interactive === InteractiveMode.NotInteractive
		) {
			this.removePointerListener(entityId)
		}
	}
	...
	private addPointerListener(
		...
		const listener = pointerEventsSystem.onPointerDown(
			...
			() => {
				console.log(
					`🎯 PointerEventManager: Pointer event triggered for entity: ${entityId} (${questEntity.name})`
				)
				...
				const commands = this.questEngine.processInteraction(interactionType, {
					id: entityId,
				})
				console.log(
				...
				console.log(
					`✅ PointerEventManager: Finished executing commands for ${interactionType} on ${entityId}`
				)
			}
		)
		...
		if (this.debugMode) {
			console.log(
				`Added pointer listener for ${entityId} (${questEntity.name})`
			)
		}
	}
