...
export function inputListenerSystem() {
  ...
  if (PPET && PPEP) {
    if (activeNPC) {
      ...
      if (!dialogData || !dialogData.visible || Date.now() - dialogData.openTime < 100) return
      if (dialogData.typing) {
        rushText(activeNPC)
  ...
  if (PET && PEP) {
    if (activeNPC) {
      ...
      if (dialogData.isQuestion) {
        // E key now activates the first button instead of second
  ...
  // Removed F key (SET/SEP) functionality as requested
