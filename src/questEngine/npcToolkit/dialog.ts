...
import {
  leftClickSection,
  leftClickSectionbBlack,
  lightTheme,
  section,
  skipButtonSection,
  skipButtonSectionBlack
} from './ui'
...
export function addDialog(npc: Entity, sound?: string, defaultPortrait?: ImageData) {
  ...
  npcDialogComponent.set(npc, {
    ...
    portraitHeight:
      defaultPortrait && defaultPortrait.height ? defaultPortrait.height * UIscaleMultiplier : portraitScale,
    portraitX:
    ...
    portraitY:
      defaultPortrait && defaultPortrait.offsetY
        ...
        : portraitYPos,
    portaitSHeight:
      defaultPortrait && defaultPortrait.section && defaultPortrait.section.sourceHeight
        ? defaultPortrait.section.sourceHeight
        : 256,
    portraitSWidth:
      defaultPortrait && defaultPortrait.section && defaultPortrait.section.sourceWidth
        ? defaultPortrait.section.sourceWidth
        : 256,
    portaitSTop:
      defaultPortrait && defaultPortrait.section && defaultPortrait.section.sourceTop
        ? defaultPortrait.section.sourceTop
        : 0,
    portraitSLeft:
      defaultPortrait && defaultPortrait.section && defaultPortrait.section.sourceLeft
        ? defaultPortrait.section.sourceLeft
        : 0
  })
}
...
export function getSkipableTheme() {
  return !isActiveNpcSet()
    ? skipButtonSectionBlack
    : npcDataComponent.get(activeNPC).theme == lightTheme
    ? skipButtonSectionBlack
    : skipButtonSection
}
...
export function getImage() {
  const npcComp = npcDialogComponent.get(activeNPC)
  if (!npcComp) return ''

  let texture = npcComp.dialogImageTexture
  if (!texture) return ''
...
export function displayButton(button: number) {
  return !isActiveNpcSet()
    ? false
    : npcDialogComponent.get(activeNPC).isQuestion && npcDialogComponent.get(activeNPC).buttons >= button
}

export function displayFirstButtonContainer() {
  return !isActiveNpcSet()
    ? false
    : npcDialogComponent.get(activeNPC).isQuestion && npcDialogComponent.get(activeNPC).buttons >= 1
}

export function displaySecondButtonContainer() {
  return !isActiveNpcSet()
    ? false
    : npcDialogComponent.get(activeNPC).isQuestion && npcDialogComponent.get(activeNPC).buttons >= 3
}
...
export function getTextPosition() {
  ...
  return !isActiveNpcSet()
    ? { top: 0, left: 0 }
    : {
        top: npcDialogComponent.get(activeNPC).margin + npcDialogComponent.get(activeNPC).positionY,
        left: npcDialogComponent.get(activeNPC).positionX
      }
}
...
function beginTyping(npc: Entity) {
  ...
  if (currentText.portrait) {
    ...
    dialogData.portraitWidth = currentText.portrait.width
      ? currentText.portrait.width * UIscaleMultiplier
      : portraitScale

    dialogData.portraitHeight = currentText.portrait.height
  ...
  if (dialogData.script[dialogData.index].image) {
    ...
    dialogData.imageX = dialogData.script[dialogData.index].image.offsetX
      ? dialogData.script[dialogData.index].image.offsetX * UIscaleMultiplier + imageXPos
      : -40
    dialogData.imageY = dialogData.script[dialogData.index].image.offsetY
      ? dialogData.script[dialogData.index].image.offsetY * UIscaleMultiplier + imageYPos
      : 40
    dialogData.imageHeight = dialogData.script[dialogData.index].image.height
      ? dialogData.script[dialogData.index].image.height * UIscaleMultiplier
      : imageScale
    dialogData.imageWidth = dialogData.script[dialogData.index].image.width
      ? dialogData.script[dialogData.index].image.width * UIscaleMultiplier
      : imageScale
    dialogData.displayImage = true
  }

  dialogData.positionX = dialogData.script[dialogData.index].offsetX
    ? dialogData.script[dialogData.index].offsetX * UIscaleMultiplier
    : '22%'
  dialogData.positionY = dialogData.script[dialogData.index].offsetY
    ? dialogData.script[dialogData.index].offsetY * UIscaleMultiplier + textYPos
    : textYPos
...
export function skipDialogs(npc: Entity) {
  ...
  while (
    ...
    if (dialogData.script[dialogData.index].skipable && dialogData.script[dialogData.index].isEndOfDialog) {
...
export function getImageAtlasMapping(data?: ImageAtlasData): number[] {
  ...
  return [
    sourceLeft / atlasWidth,
    (atlasHeight - sourceTop - sourceHeight) / atlasHeight,
    sourceLeft / atlasWidth,
    (atlasHeight - sourceTop) / atlasHeight,
    (sourceLeft + sourceWidth) / atlasWidth,
    (atlasHeight - sourceTop) / atlasHeight,
    (sourceLeft + sourceWidth) / atlasWidth,
    (atlasHeight - sourceTop - sourceHeight) / atlasHeight
  ]
...
export function closeTag(dialogData: any, newChars: number) {
  ...
  while (
    dialogData.visibleChars < dialogData.fullText.length &&
    dialogData.fullText.substr(dialogData.visibleChars - 1, 1) != '>'
  ) {
