...
import { Location, EntityState } from '../../questEditor/models'
...
export class LocationAdapter {
  ...
  constructor(entityManager: EntityManager, questEngine: any, debugMode: boolean = false) {
  ...
  private hideLocationEntities(locationId: string): QuestCommand[] {
    ...
    if (this.debugMode) {
      console.log(`Hiding ${locationEntityIds.length} entities for location ${locationId}`)
    }
  ...
  private showLocationEntities(location: Location): QuestCommand[] {
    ...
    for (const npc of location.npcs) {
      ...
      if (npc.state === 'world') {
        ...
        if (npcDefinition) {
          if (this.debugMode) {
            console.log(`Found NPC definition for ${npc.id}: ${npcDefinition.name}`)
          }
        } else {
          if (this.debugMode) {
        ...
        const npcData = {
          ...
          onActivate: (other: Entity) => {
            ...
            const commands = this.questEngine.processInteraction('clickNPC', { id: npc.id })
        ...
        console.log(`Looking for NPC entity: ${npcEntityName} for NPC: ${npc.id}`)
        const npcEntity = this.entityManager.getEntityByDecentralandName(npcEntityName)
        if (npcEntity) {
    ...
    if (this.debugMode) {
      const entityCount = location.items.length + location.npcs.length + location.portals.length
      console.log(`Showing entities for location ${location.id}: ${entityCount} total entities`)
    }
  ...
  private getEntitiesForLocation(locationId: string): string[] {
    const location = EMBEDDED_LOCATIONS.find((loc: any) => loc.id === locationId)
    if (!location) {
      if (this.debugMode) {
    ...
    if (this.debugMode) {
      console.log(`LocationAdapter: Found ${entityIds.length} entities for location ${locationId}:`, entityIds)
    }
   ...
   */
  updateEntityState(entityId: string, state: 'world' | 'inventory' | 'void'): QuestCommand[] {
    const commands: QuestCommand[] = []
