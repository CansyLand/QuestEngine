...
export const QuestStepSelector: React.FC<QuestStepSelectorProps> = ({
	...
	const questStepsForSelectedQuest = selectedQuest
		? availableQuestSteps.filter((step) =>
				selectedQuest.steps.some((s: QuestStep) => s.id === step.id)
		  )
		: []

	// Find the NPC associated with this dialogue
	const dialogueNpc = dialogueNpcId
		? availableNpcs.find((n) => n.id === dialogueNpcId)
		: null
	...
	return (
		<div className='modal-overlay' onClick={onClose}>
			<div
				className='modal-content quest-step-selector'
					...
					<div className='steps-column'>
						...
						{selectedQuest ? (
															...
															<ul>
																{assignedInfos.map((info, index) => (
																	...
																	>
																		...
																		{info.npc && !info.npc.image && (
																			<div className='assigned-npc-portrait no-image'>
																				🖼️
																			</div>
																		)}
																		{!info.npc && (
							...
							<div className='no-selection'>
								Select a quest to see its steps
							</div>
						)}
					</div>
				</div>
						...
						<ul>
							{selectedQuestStepIds.map((stepId) => {
								...
								const quest = availableQuests.find((q) =>
									q.steps.some((s: QuestStep) => s.id === stepId)
								)
								return (
									<li key={stepId}>
										...
										<button
											...
											onClick={() => onDeselectQuestStep(stepId)}
										>
											×
										</button>
									</li>
