...
import { Portal, InteractiveMode, EntityState, Location } from '@/core/models/types'
  ...
  return (
    <div className="edit-form">
      <div className="form-group">
        <label>Name:</label>
        <input type="text" value={portal.name} onChange={(e) => onUpdate({ name: e.target.value })} />
      </div>
      <div className="form-group">
        <div className="two-column-layout">
          <div className="column">
            <label>Image:</label>
            <ImagePicker value={portal.image} onChange={(value) => onUpdate({ image: value })} />
          </div>
          <div className="column">
            <label>Destination Location:</label>
            <div className="location-picker-square" onClick={onOpenLocationSelector}>
              {selectedLocation?.image ? (
                <img
                  ...
                  onError={(e) => {
                    ...
                    const fallback = e.currentTarget.parentElement?.querySelector('.location-text') as HTMLElement
                    if (fallback) fallback.style.display = 'flex'
                  }}
                />
              ) : null}
              <div className="location-text" style={{ display: selectedLocation?.image ? 'none' : 'flex' }}>
                Select Location
              </div>
            </div>
          </div>
      ...
      <div className="form-group">
        ...
        <select
          ...
          onChange={(e) => onUpdate({ interactive: e.target.value as InteractiveMode })}
        >
      ...
      <div className="form-group">
        <label>State:</label>
        <select value={portal.state} onChange={(e) => onUpdate({ state: e.target.value as EntityState })}>
