...
import { Game, Item, NPC, Portal, Location } from '@/core/models/types'
import {
	EntityTooltip,
	TooltipEntityBase,
...
export const LinksPanel: React.FC<LinksPanelProps> = ({ gameData }) => {
	...
	useEffect(() => {
		const loadDclEntities = async () => {
			console.log('LinksPanel: Loading DCL entities...')
			try {
				const response = await apiRequest('/entityLinks')
				console.log('LinksPanel: API response:', response)
				if (response.success && response.data) {
					const entities: DclEntity[] = Object.entries(response.data).map(
					...
					console.log('LinksPanel: Loaded', entities.length, 'entities')
					setDclEntities(entities)
				} else {
					console.error(
						'LinksPanel: Failed to load DCL entities:',
						response.error
					)
					// Retry after a delay
					setTimeout(() => {
						console.log('LinksPanel: Retrying...')
						loadDclEntities()
					}, 1000)
				}
			} catch (error) {
				console.error('LinksPanel: Failed to load DCL entities:', error)
				// Retry after a delay
				setTimeout(() => {
					console.log('LinksPanel: Retrying...')
					loadDclEntities()
		...
		// Load initially with a small delay to ensure project setup is complete
		setTimeout(() => {
			loadDclEntities()
		}, 500)
	...
	useEffect(() => {
		...
		const locations: QuestEntity[] = gameData.locations.map(
			(location: any) => ({
				id: location.id,
				name: location.name,
		...
		const linkedIds = new Set(
			dclEntities.map((e) => e.questEntityId).filter(Boolean)
		)
		const unlinked = allQuestEntities.filter(
	...
	const handleDrop = async (e: React.DragEvent, dclEntityId: string) => {
		...
		try {
			...
			if (response.success) {
				...
				setSaveError(
					`Failed to link entity: ${response.error || 'Unknown error'}`
				)
			}
		} catch (error) {
	...
	const handleUnlink = async (dclEntityId: string) => {
		...
		try {
			...
			if (response.success) {
				setDclEntities((prev) =>
					prev.map((e) =>
						e.id === dclEntityId ? { ...e, questEntityId: null } : e
					)
				)
				setHasChanges(true)
			} else {
				setSaveError(
					`Failed to unlink entity: ${response.error || 'Unknown error'}`
				)
			}
		} catch (error) {
	...
	const uniqueParents = Array.from(
		new Set(
			dclEntities.map((entity) => {
				...
				const parentEntity = dclEntities.find(
					(parent) => parent.id === entity.parent.toString()
				)
	...
	const filteredEntities =
		parentFilter === 'all'
			...
			: dclEntities.filter((entity) => {
					...
					return (
						(parentEntity ? parentEntity.name : entity.parent.toString()) ===
						parentFilter
					)
			  })
		...
		<div className='links-panel'>
			<div className='links-left'>
				<div className='dcl-header'>
					...
					{hasChanges && !isSaving && !saveError && (
						<div className='save-status success'>Changes saved</div>
					)}
						...
						>
							<div className='dcl-card-content'>
								<div className='dcl-image-section'>
									...
									entity.questEntityId !== draggingItemId ? (
										<div
			...
			<div className='links-right'>
				...
				<div className='entity-tabs'>
					<button
						className={`entity-tab ${activeTab === 'all' ? 'active' : ''}`}
						onClick={() => setActiveTab('all')}
					>
						All ({unlinkedCount})
					</button>
					<button
						className={`entity-tab ${activeTab === 'item' ? 'active' : ''}`}
					...
					<button
						className={`entity-tab ${activeTab === 'npc' ? 'active' : ''}`}
					...
					>
						Portals (
						{unlinkedItems.filter((item) => item.type === 'portal').length})
					</button>
					<button
					...
					>
						Locations (
						{unlinkedItems.filter((item) => item.type === 'location').length})
					</button>
				</div>
				<EntityTooltip
					...
					<div
						...
						onDrop={async (e) => {
							...
							if (draggedItemId) {
								...
								const dclEntity = dclEntities.find(
									(entity) => entity.questEntityId === draggedItemId
								)
								if (dclEntity) {
