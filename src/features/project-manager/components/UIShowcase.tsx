			...
			>
				...
				>
					...
					>
						...
						<p>
							It can contain any content and has a close button in the header.
						</p>
				...
				>
					...
					>
						Cards representing Decentraland (DCL) entities that can be linked to
						quest entities. Shows entity information and linked quest items.
					</p>
				...
				>
					...
					<ArrayField
						...
						renderItem={(item, index, onRemove) => (
							<ItemDisplay entity={item} variant='compact' />
						)}
				...
				>
					...
					>
						...
						<button
							className='modal-button save'
							onClick={() => setNpcSelectorOpen(true)}
						>
							NPC Selector
						</button>
						<button
					...
					<UniversalSelector
						...
						onMultiSelect={(items) => {
							showNotification(
								`Selected ${items.length} items: ${items
									.map((i) => i.name)
									.join(', ')}`,
								'success'
							)
				...
				>
					...
					<p>
						Access this page directly via URL:{' '}
						<span style={{ color: '#00ffff' }}>/ui-showcase</span>
					</p>
				</footer>
			</div>
		</div>
