	...
	const defaultRenderItem = (item: T, isSelected: boolean) => (
		<div className={`selector-item ${isSelected ? 'selected' : ''}`}>
			<EntityDisplay
				entity={item}
				variant='selector'
				className='selector-entity-display'
			/>
		</div>
	)
		...
		<div className='selector-footer'>
			{multiSelect && tempSelectedItems.length > 0 && (
				<div className='selection-info'>
					{tempSelectedItems.length} item
					{tempSelectedItems.length !== 1 ? 's' : ''} selected
				</div>
			)}
			<div className='selector-buttons'>
				...
				{multiSelect && tempSelectedItems.length > 0 && (
					<button
						...
						onClick={() => onMultiSelect && onMultiSelect(tempSelectedItems)}
					>
