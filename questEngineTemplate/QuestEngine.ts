...
export class QuestEngine {
	...
	constructor(
		...
		this.locationAdapter = new LocationAdapter(
			sceneController.getEntityManager(),
			this,
			debugMode
		)
	...
	private handleNPCInteraction(npcId: string): QuestCommand[] {
		...
		if (dialogueSequence) {
			...
			if (this.debugMode) {
				console.log(
					`Looking for NPC entity: ${npcEntityName} for dialogue with ${npcId}`
				)
			}
			...
			if (npcEntity) {
				...
				if (this.debugMode) {
					console.log(
						`Started NPC dialogue for ${npcId}: ${dialogueSequence.id}`
					)
				}
	...
	private executeAction(action: Action): QuestCommand[] {
		...
		switch (action.type) {
			...
			case ActionType.SpawnEntity:
				...
				this.pointerEventManager.updateEntityPointerEvents(
					action.params.entityId
				)
				console.log(
					`✅ Spawn entity command created for: ${action.params.entityId}`
				)
				break
		...
		return [
			{
				type: QuestCommandType.Log,
				params: {
					message: `Updated interactivity for ${entityId} to ${interactiveMode}`,
				},
			},
		]
	}
	...
	executeCommands(commands: QuestCommand[]): void {
		console.log(
			`🚀 QuestEngine: executeCommands called with ${commands.length} commands`
		)
