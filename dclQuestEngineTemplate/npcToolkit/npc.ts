...
export function create(transform: any, data: NPCData) {
  ...
  return initializeNPC(npc, data)
}

export function initializeExistingEntity(existingEntity: Entity, data: NPCData) {
  return initializeNPC(existingEntity, data)
}

function initializeNPC(npc: Entity, data: NPCData) {
...
function addNPCBones(npc: Entity, data: NPCData) {
  ...
  switch (data.type) {
    case NPCType.AVATAR:
      // Only add AvatarShape if it doesn't already exist
    ...
    case NPCType.CUSTOM:
      // For existing entities, don't add GltfContainer if it already exists
      // Only add it for newly created NPCs
      ...
      // Only add Animator if it doesn't already exist
      if (!Animator.has(npc)) {
        Animator.create(npc, {
          states: [
            {
        ...
        // Entity already has Animator, just update the NPC data
        let npcData = npcDataComponent.get(npc)
        npcData.idleAnim = data && data.idleAnim ? data.idleAnim : 'Idle'
        npcData.lastPlayedAnim = npcDataComponent.get(npc).idleAnim
        ...
        if (data && data.idleAnim) {
          Animator.playSingleAnimation(npc, data.idleAnim)
        }
      }
...
export function activate(npc: Entity, other: Entity) {
  // Check if NPC data exists - if not, the NPC has been cleaned up
  let npcData = npcDataComponent.get(npc)
  if (!npcData) {
    console.log('NPC data not found, ignoring activation (NPC may have been cleaned up)')
    return
  }
...
export function handleWalkAway(npc: Entity, other: Entity) {
  ...
  let npcData = npcDataComponent.get(npc)
  if (!npcData) {
    console.log('NPC data not found, ignoring walk away (NPC may have been cleaned up)')
    return
  }
...
export function playAnimation(npc: Entity, anim: string, noLoop?: boolean, duration?: number) {
  ...
  if (
    animations.states &&
    animations.states.filter((animation: PBAnimationState) => animation.clip === anim).length == 0
  ) {
